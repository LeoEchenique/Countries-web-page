...
export default function CreateAct() {
  ...
  function handleSelect(e) {
    setCountries((current) => [
      ...current,
      e.target.value,
    ]); /* setting an array of values for one state without taking the others away */
  }
  ...
  function handleCheck(e) {
    if (e.checked === false) {
      ...
      let filtredIds = countriesId.filter(
      ...
      <div className={style.form_container}>
        ...
        <form onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className={style.box}>
            ...
            {error?.duration ? (
              <p className={style.p}>{error.duration}</p>
            ) : null}
          </div>

          <div className={style.container_comments}>
            <label className={`${style.label} ${style.labul}`}>
              {" "}
              Comments:{" "}
            </label>
            <textarea
          ...
          {actualStorage?.length ? (
            <h1 className={`${style.countries_choice} ${style.h1}`}>
          ...
          <div className={style.countries_choose}>
            {actualStorage?.length ? (
                ...
                >
                  {actualStorage.length
                    ? actualStorage.map((e) => {
                        return (
                          <>
                            <option onClick={(e) => handleClick(e)} id={e.id}>
                              {e.name}
                            </option>
                          </>
                        );
              ...
              <h1 className={style.pre_select}>
                {" "}
                Couldn't load the countries, please go back to HOME.{" "}
              </h1>
            )}

            <div
              onChange={(e) => handleCheck(e)}
              className={style.countries_selected}
            >
              {countries.length ? (
                countries.map((e) => {
                  return (
                    <div className={style.div_selectedCon}>
                      <label className={style.label_selected}>
                        <input
                          className={style.button_17}
                          onChange={(e) => handleCheck(e.target)}
                          type="checkbox"
                          checked
                          /* checked instead of defaultCheked was the bug */ value={
                            e
                          }
                        />
                        {e}
                ...
                <p className={`${style.p} ${style.texted}`}>{error.keys} </p>
              ) : actualStorage?.length ? (
                <p className={`${style.pre_select}`}>
